...
from .annotation import output_json, read_json, read_xml
...
class ImageManager:
    ...
    Args:
        ...
        annotation_format(str): "json", "coco", or "xml" for annotation format.
    """
    ...
    def _load_rects(self):
        """Load annotations from file based on format preference."""
        if self._annotation_format in ["json", "coco"]:
            # Try JSON first
    ...
    def save_annotation(self):
        """output the annotation file in the specified format."""
        if self._annotation_format == "coco":
            output_json(self._filename, self._img, self._current_rects, "coco")
        elif self._annotation_format == "json":
            output_json(self._filename, self._img, self._current_rects, "custom")
        else:
            # Fallback to XML
            from .annotation import output_xml
            output_xml(self._filename, self._img, self._current_rects)


class ImageDirManager:
    def __init__(self, dir_name, annotation_format="json"):
    ...
    def get_exist_annotation_files(self):
        """Get existing annotation files based on format preference."""
        if self._annotation_format in ["json", "coco"]:
            # Look for JSON files
            self._annotations_files = [
                file for file in os.listdir(self._dir_name) if re.match(".*\.json", file)
            ]
        else:
            # Look for XML files
            self._annotations_files = [
    ...
    def _get_next_image_helper(self, index):
        while index < len(self._files) - 1:
            ...
            # Check for annotation file based on format
            if self._annotation_format in ["json", "coco"]:
                annotation_file = f"{image_file_name}.json"
            else:
                annotation_file = f"{image_file_name}.xml"
                
            if annotation_file not in self._annotations_files:
                return index
        return None
